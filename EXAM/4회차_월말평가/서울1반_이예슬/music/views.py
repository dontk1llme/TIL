...
def review_create(request, music_pk):
    ...
    if serializer.is_valid(raise_exception=True): #유효성 검사
        ...
        return Response(serializer.data, status=status.HTTP_201_CREATED) #생성, 201코드
    else: return Response(music, status=status.HTTP_400_BAD_REQUEST) #유효성검사 불통시 데이터, 400코드
